...
def build_site(posts):
    ...
    index = []
    for p in posts_sorted:
        excerpt = strip_html(p["content"])[:250]
        index.append({
            "slug":    p["slug"],
            "title":   p["title"],
    ...
    for pg_num, pg_posts in enumerate(pages, 1):
        ...
        if pg_posts:
            ...
            excerpt = strip_html(fp["content"])[:320]
            raw = strip_html(fp["content"])
            # Pull a short poetic snippet for the green box
            poem_lines = raw[:180].strip()
            if len(raw) > 180:
                poem_lines += "..."
        ...
        for idx, p in enumerate(tile_posts):
            excerpt = strip_html(p["content"])[:200]
